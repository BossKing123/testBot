...
//		double turn2 = Math.signum(turn) * Math.pow(Math.abs(turn - limit), 2.0);
    ...
    public void SmoothDrive(double frwd, double turn) { //The finest-er drive code known to man.
    	...
    	//Get the gyro data.
		double gz = gyro.filter(Robot.measure1.angleRate);
		double ga = Robot.measure1.angle;
		double gzMax = Robot.measure1.angleRateMax;
...
//		double turn2 = Math.signum(turn) * Math.pow(Math.abs(turn - limit), 2.0);
		...
		angle += omega * Robot.measure1.mpuDeltaT;
